...
async function getBook(): Promise<Book | null> {
    const jsonPath = path.join(process.cwd(), 'src/content/hanxuema1995/book.json');
    try {
        const fileContents = await fs.readFile(jsonPath, 'utf8');
        return JSON.parse(fileContents) as Book;
    } catch (error) {
        console.warn("Book meta not found or invalid", error);
        return null;
    }
}
    ...
    if (!book) {
        return (
            <main className="flex min-h-screen flex-col items-center justify-center p-24">
                ...
                <p className="mt-4">Please run <code>npm run process:books</code> to generate content.</p>
            </main>
        );
    }
                    ...
                    {book.chapters.map(chapter => (
                        <div key={chapter.id} className="mb-24 px-4">
                            ...
                            {chapter.poems.map(poem => (
                                <PoemView key={poem.id} chapter={chapter} poem={poem} />
                            ))}
                        </div>
                    ))}
                </div>
