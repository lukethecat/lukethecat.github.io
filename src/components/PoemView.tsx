...
export const PoemView: React.FC<PoemViewProps> = ({ chapter, poem, book }) => {
    ...
    return (
        <article
            ...
            data-page-number={poem.pageNumber || ''}
            data-book-id={book.id}
            className="max-w-2xl mx-auto py-16 px-6 md:py-28 md:px-8 scroll-mt-12 [content-visibility:auto] min-h-[500px]"
        >
            {/* Poem Title */}
            <div className="mb-10 md:mb-14">
                {poem.title.includes('／') ? (
                    <>
                        <h3 className="text-2xl md:text-3xl font-semibold text-foreground font-serif leading-relaxed tracking-wide">
                            {poem.title.split('／')[0]}
                        </h3>
                        <h3 className="text-2xl md:text-3xl font-semibold text-foreground-muted font-serif leading-relaxed tracking-wide">
                            {poem.title.split('／')[1]}
                        </h3>
                    </>
                ) : (
            ...
            {/* Poem Content — refined reading typography */}
            <div className="font-serif text-lg md:text-xl text-foreground leading-[2.4] tracking-[0.05em]">
                {poem.lines.map((line, index) => (
                    <p key={index} className={`min-h-[1.5em] ${line === '' || !line.trim() ? 'h-8 md:h-10' : ''}`}>
                        ...
                        {annotations.map((ann, i) => (
                            <li key={i} className="flex text-sm text-foreground-muted leading-relaxed">
                                ...
                                <span>
                                    ...
                                    <span className="mx-1.5 text-border-hover">—</span>
                                    {ann.note}
                                </span>
                            </li>
                        ))}
