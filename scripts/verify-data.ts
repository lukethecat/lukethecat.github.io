...
function verifyData() {
    ...
    for (const book of worksData.books) {
        ...
        for (const chapter of bookData.chapters) {
            ...
            for (const poem of chapter.poems) {
                ...
                // Milestone 5: Defense against truncated extraction
                // If a poem has 3 lines or less (e.g. only a 2-line title), it's highly suspicious and usually an extraction error.
                // We allow it only if there is a specific marker or we know it's a very short poem. 
                // For this project, almost all poems are > 3 lines.
                if (validLines.length <= 3) {
                    console.warn(`[WARNING] Poem ${poem.id} in ${book.id} is extremely short (${validLines.length} lines). Possible truncation.`);
                }
    ...
    // 4. Verify Essays Markdown Formatting (OCR glued heading check)
    const essaysDir = path.join(contentDir, 'essays');
    if (fs.existsSync(essaysDir)) {
        const essayFiles = fs.readdirSync(essaysDir).filter(f => f.endsWith('.md'));
        for (const file of essayFiles) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.match(/^#{1,6}\s/)) {
                    // It's a heading. Check if it's abnormally long (e.g., > 50 characters)
                    // which indicates OCR failed to split the title from the body paragraph.
                    if (line.length > 50) {
                        console.warn(`\x1b[31m[WARN] Abnormally long heading found in ${file}:${i+1}\x1b[0m`);
                        console.warn(`       Line: "${line.substring(0, 60)}..."`);
                        assert(false, `OCR Glued Heading detected in ${file} (length ${line.length} > 50)`);
                    }
                }
            }
        }
        console.log(`Verified ${essayFiles.length} essay markdown files for OCR formatting.`);
    }
